...
agg_stats <- function(df,alpha.FCR) {
  ...
  BH_full_CI <- sapply(alpha.list,function(alpha) {
    ...
    up <- df$Y[x] + qnorm(1-alpha.2/2,mean=0,sd=1)
    low <- df$Y[x] - qnorm(1-alpha.2/2,mean=0,sd=1)
  ...
  ada_full_CI <- apply(df$AdaPT.full.object$s,2,
                       function(s) {
                         ...
                         up <- df$Y[tmp] + qnorm(1-alpha.2/2,mean=0,sd=1)
                         low <- df$Y[tmp] - qnorm(1-alpha.2/2,mean=0,sd=1)
  ...
  BH_mask_CI <- sapply(alpha.list,function(alpha) {
    ...
    up <- m + qnorm(1-alpha.FCR/2,mean=0,sd=sqrt(df$var_mask/nrej))
    low <- m - qnorm(1-alpha.FCR/2,mean=0,sd=sqrt(df$var_mask/nrej))
...
t1 <- get_summary('experiments_refresh2.R',alpha.list,2,0.1)
t2 <- get_summary('experimentsref_2_0.3',alpha.list,2,0.3)
t3 <- get_summary('experimentsref_2_0.5',alpha.list,2,0.5)
t4 <- get_summary('experimentsref_2_0.7',alpha.list,2,0.7)
t5 <- get_summary('experimentsref_2_0.9',alpha.list,2,0.9)


agg <- rbind(t1,t2,t3,t4,t5)


ds <- agg[agg$mu == 2 & agg$Splitting=="Blurred",]
...
ggsave("results_large_norm_sel.pdf")

grid.arrange(p3,p4,nrow=1,ncol=2)
ggsave("results_large_norm_inf.pdf")

grid.arrange(p1,p2,p3,p4,nrow=2,ncol=2)


p2 <- ggplot(agg[agg$Metric == "FCR" & agg$TargetLevel==0.1 &agg$mu == 2&agg$Splitting="Blurred" ,],
             aes(x = tau, y = Value, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
        panel.background = element_rect(fill = "white", colour = "black"),
        panel.grid.major = element_line(colour = "grey", linetype = "dotted"),
...
p4 <- ggplot(agg[agg$Metric == "CI Length" & agg$TargetLevel==0.1 &agg$mu == 2 ,],
             aes(x = tau, y = Value, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
p5 <- ggplot(agg[agg$Metric == "FDP" & agg$TargetLevel==0.1 &agg$mu == 3 ,],
             aes(x = tau, y = Value, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
p6 <- ggplot(agg[agg$Metric == "Power" & agg$TargetLevel==0.1 &agg$mu == 3 ,],
             aes(x = tau, y = Value, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
p7 <- ggplot(agg[agg$Metric == "FCR" & agg$TargetLevel==0.1 &agg$mu == 3 ,],
             aes(x = tau, y = Value, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
p8 <- ggplot(agg[agg$Metric == "CI Length" & agg$TargetLevel==0.1 &agg$mu == 3 & agg$Splitting == "Blurred",],
             aes(x = tau, y = Value, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
pdf("resuls_varytau.pdf")
grid.arrange(p1,p2,p3,p4,nrow=2,ncol=2)
             
             
             )
grid.arrange(p5,p6,p7,p8,nrow=2,ncol=2,top = "Blurring Procedures: mu=3,alpha_selection = 0.1, alpha_FCR = 0.2")
dev.off()
...
p2 <- ggplot(agg[agg$Metric == "FCR" & agg$TargetLevel==0.1 &agg$mu == 2,],
             aes(x = tau, y = Value, fill = Procedure,linetype=Splitting)) +
  geom_line(aes(color = Procedure,linetype=Splitting), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
p3 <- ggplot(agg[agg$Metric == "Power" & agg$TargetLevel==0.1 &agg$mu == 2,],
             aes(x = tau, y = Value, fill = Procedure,linetype=Splitting)) +
  geom_line(aes(color = Procedure,linetype=Splitting), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
p4 <- ggplot(agg[agg$Metric == "Power" & agg$TargetLevel==0.1 &agg$mu == 2,],
             aes(x = tau, y = Value, fill = Procedure,linetype=Splitting)) +
  geom_line(aes(color = Procedure,linetype=Splitting), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
...
p1 <- ggplot(data = df_FDP,
       aes(x = TargetLevel, y = FDR, fill = Procedure,linetype=Splitting)) +
  geom_line(aes(color = Procedure,linetype=Splitting), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
...
p2 <- ggplot(data = df_power,
       aes(x = TargetLevel, y = Power, fill = Procedure,linetype=Splitting)) +
  geom_line(aes(color = Procedure,linetype=Splitting), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
        panel.background = element_rect(fill = "white", colour = "black"),
...
p3 <- ggplot(data = df_MSE,
       aes(x = TargetLevel, y = MSE, fill = Procedure,linetype=Splitting)) +
  geom_line(aes(color = Procedure,linetype=Splitting), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(
        panel.background = element_rect(fill = "white", colour = "black"),
  ...
  xlab("Target Level")+
  ylab("MSE of average selected signal strength")

png("STAR_experiment_graph1.png")
grid.arrange(p1,p2,p3,nrow=1,ncol=3)
dev.off()


p4 <- ggplot(data = subset(df_FCR, Splitting == "Full"),
       aes(x = TargetLevel, y = FCR, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
        panel.background = element_rect(fill = "white", colour = "black"),
...
p5 <- ggplot(data = subset(df_FCR, Splitting == "Blurred"),
             aes(x = TargetLevel, y = FCR)) +
  geom_line(aes(color = Procedure,linetype=Spli), linetype='dashed',size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
        panel.background = element_rect(fill = "white", colour = "black"),
        panel.grid.major = element_line(colour = "grey", linetype = "dotted"),
        ...
        legend.position="none") +
  xlab("Target Level") +
  ylab("Miscoverage rate  of avg. signal strength") +
  ggtitle("Blurred Data") +
  ylim(0,1)



p6 <- ggplot(data =  subset(df_CI_length, Splitting == "Full"),
             aes(x = TargetLevel, y = `CI Length`, fill = Procedure)) +
  geom_line(aes(color = Procedure), size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
        panel.background = element_rect(fill = "white", colour = "black"),
...
p7 <- ggplot(data = subset(df_CI_length, Splitting == "Blurred"),
             aes(x = TargetLevel, y =`CI Length`)) +
  geom_line(aes(color = Procedure), linetype='dashed',size = 0.8) +
  geom_point(aes(shape = Procedure, color = Procedure), size = 1.5) +
  theme(legend.title = element_blank(),
        panel.background = element_rect(fill = "white", colour = "black"),
        panel.grid.major = element_line(colour = "grey", linetype = "dotted"),
